...
<div class="mt-2">
    <form id="question_type_image" action="" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="file" id="files" class="form-control" style='color:{{$lesson->color2}} !important;border-color:{{$lesson->color1}};outline-color:{{$lesson->color1}};font-size:{{$firstFontSize}};font-style:{{$firstStyle}};font-family:{{$firstFamily}};' name="files[]" accept=<?php $cont=$question->content;
...
<script>
    ...
    $("body").append(`<style>#q_<?php echo $question->id ?> #files::file-selector-button{<?php echo 'font-size:'.$firstFontSize.';font-style:'.$firstStyle.';font-family:'.$firstFamily.';' ?>;color:{{$lesson->color2}} !important;border-color:{{$lesson->color1}};background:{{$question->color1 == NULL ? $lesson->color1 : $lesson->color1}};}</style>`)  
</script>
