...
class LessonsController extends Controller
{
    ...
    public function get_answers_fill(Request $request){
        ...
        foreach ($qu as $key => $value) {
            $answers = DB::table('user_answer')->select('id', 'q_id', 'answer', 'score')->where('user_id', $user_id)->where('test_id', $test_id)->where('question_id', $value->question_id)->get();
            $data = [];
            ...
            if($inputText->help_info != ""){
                ...
                if (preg_match($patternA, $inputText->help_info, $matches)) {
                    ...
                    if(preg_match($patternB, $inputText->help_info, $matches)){
                        ...
                        $help['content'] =  "<p class='modelTextStyle'>". strip_tags($inputText->help_info) ."</p>";
                        $help['step'] = 3;
                    }
                }
            }else{
